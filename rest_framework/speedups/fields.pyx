...
cdef class empty:
    """
    This cdef class is used to represent no data being provided for a given input
    or output value.
...
cdef public bint is_simple_callable(obj):
    """
...
cdef public get_attribute(instance, list attrs):
    """
...
cdef public void set_value(dict dictionary, keys, value):
    """
...
cdef class CreateOnlyDefault:
    """
    This cdef class may be used to provide default values that are only used
    for create operations, but that do not return any value for update
    operations.
    """
    cdef public bint is_update
...
cdef class Field(object):
    ...
    cpdef bint read_only
    cpdef bint write_only
    cpdef bint required
    cpdef bint allow_null

    def __init__(self, bint read_only=False, bint write_only=False,
                 bint required=None, default=empty, initial=empty, source=None,
                 label=None, unicode help_text=None, style=None,
                 dict error_messages=None, list validators=None, bint allow_null=False):
    ...
    def bind(self, unicode field_name, Field parent):
        """
    ...
    def get_value(self, dict dictionary):
        """
    ...
    def __cinit__(cls, *args, **kwargs):
        ...
        """
        instance = super(Field, cls).__cinit__(cls)
        instance._args = args
...
cdef class BooleanField(Field):
    ...
    cpdef bint to_internal_value(self, data):
    ...
    cpdef bint to_representation(self, value):
...
cdef class NullBooleanField(Field):
    default_error_messages = {
        'invalid': _('`{input}` is not a valid boolean.')
    }
    initial = None
...
cdef class CharField(Field):
    ...
    cpdef str to_internal_value(self, data):
        return six.text_type(data)

    cpdef str to_representation(self, value):
        return six.text_type(value)


cdef class EmailField(CharField):
    default_error_messages = {
...
cdef class RegexField(CharField):
    default_error_messages = {
...
cdef class SlugField(CharField):
    default_error_messages = {
...
cdef class IntegerField(Field):
    ...
    cpdef int to_internal_value(self, data):
    ...
    cpdef int to_representation(self, value):
        return int(value)
...
cdef class FloatField(Field):
    ...
    cpdef float to_internal_value(self, data):
        if isinstance(data, six.text_type) and len(data) > self.MAX_STRING_LENGTH:
            self.fail('max_string_length')
    ...
    cpdef float to_representation(self, value):
        return float(value)
...
cdef class DecimalField(Field):
    ...
    cdef public int max_digits
    cdef public int decimal_places

    def __init__(self, int max_digits, int decimal_places, bint coerce_to_string=None, double max_value=None, double min_value=None, **kwargs):
...
cdef class ImageField(FileField):
    default_error_messages = {
        'invalid_image': _(
...
cdef class _UnvalidatedField(Field):
    def __init__(self, *args, **kwargs):
...
cdef class ReadOnlyField(Field):
    """
    ...
    cdef class ExampleSerializer(self):
...
cdef class SerializerMethodField(Field):
    """
